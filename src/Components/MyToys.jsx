    ...
    fetch(`http://localhost:5000/addstoy/${id}`, {
      ...
      .then(data => {
        ...
        if (data.modifiedCount > 0) {
          // update state
          const remaining = utoy.filter(ut => ut._id !== id)
          const updated = utoy.find(ut => ut._id === id)
      ...
      <div>
        {
          utoy.map(t => <div key={t._id}>
            ...
            <div className="bg-rose-900 shadow-xl rounded-sm  mt-10 text-center w-fit flex">
              ...
              {
                utoy.status === "confirm" ? <button className="bg-rose-700 hover:bg-rose-700 text-white py-2 px-4 rounded">
                  <p className='font-bold text-3xl text-yellow-400'>UPDATE THE TOY</p>
                </button>

                  :

                  <button onClick={() => handleConfirm(t._id)} className="bg-rose-700 hover:bg-rose-700 text-white py-2 px-4 rounded">
                    <p className='font-bold text-3xl text-yellow-400'>UPDATE THE TOY</p>
                  </button>
              }
